 ...
 */
const helpMessage: Middleware<SlackCommandMiddlewareArgs> = async ({
    say,
    message,
    context,
}) => {
 ...
 */
const askIfShouldIntroduce: Middleware<SlackEventMiddlewareArgs> = async ({
    event,
    client,
}) => {
    const auth = await client.auth.test();
    if (!auth.ok || auth.user_id !== event.user || !event.inviter) {
        return;
    }
 ...
 */
const introduceBot: Middleware<SlackActionMiddlewareArgs<'message'>> = async ({
    ack,
    say,
    respond,
    context,
