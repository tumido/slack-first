 ...
 */
const tagOnCallAction: Middleware<
    SlackActionMiddlewareArgs<'message'> | SlackShortcutMiddlewareArgs
> = async ({ body, payload, ack, client, respond, say, context }) => {
    await ack();
 ...
 */
const tagOnCallCommand: Middleware<SlackCommandMiddlewareArgs> = async ({
    ack,
    respond,
    context,
}) => {
    ...
    await respond(
        `<@${context.onCallUser}> is the support person of the day! Please ask him in a DM or post a message in <#${context.config.supportChannelId}> channel`
    );
};
